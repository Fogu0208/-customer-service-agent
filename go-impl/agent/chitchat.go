...
可以简要介绍你能帮忙查询理财产品、退款政策、开户流程、订单状态，以及协助创建工单。不要编造具体产品或订单数据。`
			if reply, err := a.llm.ChatHistory(system, toLLMHistory(state.History), state.UserMessage); err == nil && reply != "" {
				state.SubResults["chitchat"] = reply
				return state
			}
		}

		state.SubResults["chitchat"] = "您好！我是 Smart CS 智能客服，可以帮您咨询理财产品、退款政策、开户流程、订单查询，或协助创建工单。请问需要什么帮助？"
		return state
	})
}
